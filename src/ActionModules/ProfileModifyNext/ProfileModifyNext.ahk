...
class ProfileModifyNext {
    ...
    static db2 := useDateBase(this.dbSetting)
    ...
    static USE(App) {
        ...
        if (!FileExist(this.db2.backup . "\" . SubStr(yesterday, 1, 6) . "\" . yesterday . "_backup.json")) {
            this.db2.createBackup(yesterday)
        }
        
        PMN_App(App, this.title, this.db, this.db2, this.identifier)
    }
}